...
export class Backend implements backendInterface {
    ...
    async getExperience(): Promise<Array<{
        period: string;
        description: string;
        company: string;
        position: string;
    ...
    async getProjectDetails(arg0: string): Promise<{
        id: string;
        title: string;
        role: string;
        description: string;
        ...
        if (this.processError) {
            try {
                const result = await this.actor.getProjectDetails(arg0);
    ...
    async getProjectVideos(): Promise<Array<[string, string]>> {
        if (this.processError) {
            try {
                const result = await this.actor.getProjectVideos();
                return result;
            } catch (e) {
                this.processError(e);
                throw new Error("unreachable");
...
function from_candid_opt_n10(uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: [] | [{
        name: string;
    }]): {
    name: string;
} | null {
...
function from_candid_opt_n13(uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: [] | [{
        id: string;
        title: string;
        role: string;
        description: string;
